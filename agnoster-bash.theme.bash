...
#
# Updated & fixed by Daniel Igel 3/15/18
#
# # README
#
# In order for this theme to render correctly, you will need a
# [Powerline-patched font](https://gist.github.com/1595572).
# Recommend from Erik Selber: https://github.com/powerline/fonts.git
...
prompt_segment() {
    ...
    CURRENT_BG=$1
    [[ -n $3 ]] && PRLEN=$(($PRLEN + ${#3})) && PR="$PR$3"
}

# Exactly the same for the right prompt
prompt_segment_right() {
    ...
    if [[ $CURRENT_BG != NONE && $1 != $CURRENT_BG ]]; then
        declare -a intermediate=($(fg_color $CURRENT_BG) $(bg_color $1))
        debug "pre prompt " $(ansi intermediate[@])
        PRight="$SEGMENT_SEPARATOR$PRight $(ansi intermediate[@])"
        debug "post prompt " $(ansi codes[@])
    ...
    CURRENT_BG=$1
    [[ -n $3 ]] && PRIGHTLEN=$(($PRIGHTLEN+${#3})) &&PRight="$PRight$3"
...
# One final symbol before the end
prompt_symbol() {
...
prompt_time() {
    local time=`date +%H:%M`
    prompt_segment_right blue black "$time"
}
...
prompt_status() {
    ...
    [[ -n "$symbols" ]] && PRIGHTLEN=$(($PRIGHTLEN+${#symbols})) && PRight="$PRight$symbols"
}
...
prompt_right() {
    let COL=$(tput cols)-${PRIGHTLEN}-${PRLEN}+3
    PR="$PR$(printf %s%${COL}s$PRight)"
}
...
set_bash_prompt() {
    ...
    PRLEN=0
    PRight="$(ansi_single $(text_effect reset))"
    PRIGHTLEN=0
    PRStatus=""
    build_prompt
